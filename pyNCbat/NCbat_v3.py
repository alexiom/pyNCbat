...
class ModelCase(ElectricityModel):
    ...
    def __init__(self, Nstor: int, caseID:Union[int,str] = 1, NstorMax:int = 2, 
                 StorPrfx:tuple = ("Bat","LDS","SDS"),
                 param_file:str='temp_param.csv') -> None:
        ...
        filePar = self.PATH + param_file       # File Names
        dfPar   = pd.read_csv(filePar)         # Read In Parameter Data
        dfVars  = dfPar.columns                # Store Variable Names
        dfPar   = dfPar.iloc[caseID-1]         # Store Case Parameters 

        # Get Storage Technologies
        storVars = [v for v in dfVars if v.startswith("cap") and v.endswith(StorPrfx)] # get all storage techs
        storVars = sorted([v.replace("cap", "") for v in storVars]) # remove cap prefix and sort alphabetically
        
        # If only one alpha parameter is supplied, remove long duration storage
        if Nstor == 1:
            storVars = [v for v in storVars if not v.endswith(("LDS"))]
